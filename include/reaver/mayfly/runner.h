...
namespace mayfly
{
    inline namespace _v1
    {
        ...
        {
        public:
            runner(std::size_t threads = 1, std::size_t timeout = 60, std::optional<std::string> test_name = {})
                : _threads{ threads }, _timeout{ timeout }, _test_name{ std::move(test_name) }
            {
            }
        ...
        {
        public:
            subprocess_runner(std::string executable, std::size_t threads = 1, std::size_t timeout = 60, std::optional<std::string> test_name = {})
                : runner{ threads, timeout, std::move(test_name) }, _executable{ std::move(executable) }
            {
            }
        ...
        private:
            ...
            void _handle_suite(thread_pool * pool, const suite & s, const reporter & rep, std::vector<std::reference_wrapper<const suite>> suite_stack = {})
            {
                ...
                if (_test_name
                    && boost::range::search(*_test_name, boost::join(fmap(suite_stack, [](auto && s) { return s.get().name(); }), "/")) != _test_name->begin())
                {
                    return;
                }
                    ...
                    for (const auto & test : s)
                    {
                        ...
                        auto callback = [=, &rep]() {
                            ...
                            else
                            {
                                ...
                                _failed.push_back(std::make_pair(
                                    result.status, boost::join(fmap(suite_stack, [](auto && s) { return s.get().name(); }), "/") + "/" + test.name()));
                            }
                        };
            ...
            testcase_result _run_test(const testcase & t,
                const suite & s,
                const reporter & rep,
                std::vector<std::reference_wrapper<const suite>> suite_stack) const
            {
                testcase_result result;
                result.name = t.name();
                result.status = testcase_status::passed;
                ...
                std::vector<std::string> args{
                    _executable, "--test", boost::join(fmap(suite_stack, [](auto && s) { return s.get().name(); }), "/") + "/" + t.name(), "-r", "subprocess"
                };
        ...
        namespace options
        {
            ...
            new_opt_ext(timeout, std::size_t, opt_name_desc("timeout,l", "specify the timeout for tests (in seconds)");
        ...
        inline int run(const std::vector<suite> & suites, int argc, char ** argv)
        {
            ...
            // this is here only because the options library in reaverlib doesn't yet know how to print help
            ...
            auto parsed = reaver::options::parse_argv(argc,
                argv,
                tpl::vector<options::help,
                    options::version,
                    options::tasks,
                    options::test,
                    options::reporter,
            ...
            default_runner(std::make_unique<subprocess_runner>(executable, parsed.get<options::tasks>(), parsed.get<options::timeout>(), test_name))(
                suites, reporter);
